...
#define kMaxImageWidth 500  //不建议设置太大，太大的话会导致图片加载过慢
...
- (NSMutableArray<ChoosePhotoModel *> *)getPhotoAblumList{
    NSMutableArray<ChoosePhotoModel *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if(collection.assetCollectionSubtype != 202 && collection.assetCollectionSubtype < 212){
            NSArray<PHAsset *> *assets = [self getAllAssetInPhotoAblumWithAscending:false withCollection:collection];
            if (assets.count > 0) {
                ChoosePhotoModel *ablum = [[ChoosePhotoModel alloc] init];
